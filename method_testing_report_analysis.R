...
predict_test <- function(impute_method, prop=.25, kval = 5,  n=3, seeded = TRUE, weighted = FALSE, append = FALSE) {
  ...
  for (h in seq(n)) {
    ...
    # Population Aggregate states on prop pay
    ...
    pred_SE_results <- replicate_weights_SE(df, 'prop_pay', prop)
    act_SE_results <- replicate_weights_SE(df, 'prop_pay_act', prop)
    pred_SE <- pred_SE_results[['std_error']]
    pred_CI_low <- pred_SE_results[['CI_low']]
    pred_CI_high <- pred_SE_results[['CI_high']]
    act_SE <- act_SE_results[['std_error']]
    ...
    df['result'] <- with(df, ifelse(prop_pay == prop_pay_act, 1, 0))
    ...
    df['result_dist'] <- with(df, abs(prop_pay - prop_pay_act))
    dist_accuracy <- sum(df$result*df$weight, na.rm = TRUE)
